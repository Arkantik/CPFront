...
export default function AddCountryForm({ refetch }: { refetch: () => void }) {
    const [createCountry, error] = useAddCountryMutation({ errorPolicy: "all" });
    ...
    const handleSubmit = (event: React.FormEvent) => {
        ...
        createCountry({ variables: { data: formJSON } }).then(refetch);
    }
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col lg:flex-row gap-4 p-4 border rounded items-end w-full">
            <label htmlFor="name" className="flex flex-col w-full">
                Name*:
                <input type="text" name="name" minLength={2} maxLength={50} className="rounded border p-2" />
            </label>
            <label htmlFor="emoji" className="flex flex-col w-full">
                Emoji*:
                <input type="text" name="emoji" maxLength={4} className="rounded border p-2" />
            </label>
            <label htmlFor="code" className="flex flex-col w-full">
                Code*:
                <input type="text" name="code" minLength={2} maxLength={3} className="rounded border p-2" />
            </label>
            ...
            <button type="submit" className="bg-pink-500 rounded p-2 text-white font-bold w-full mt-4 lg:mt-0">Add</button>
        </form>
    );
};
